...
register.controller('AdvertFormRegisterCtrl', function ($scope, $http, $window, $log, $mdToast, registerFactory) {
	...
	$scope.showActionToast = function (msg, btn) {
		...
		var toast = $mdToast.simple().textContent(msg).action(btn).highlightAction(true).hideDelay(3000).position(pinTo);
	...
	$scope.$watch('register', function (newValue, oldValue) {}, true);
	...
	$scope.registerFormSubmit = function (isValid) {
		...
		registerFactory.register_user(registerdata).then(function successCallback(resp) {
			var data = resp.data;
			$scope.activated = false;
			...
			if (resp.type == 'success') $window.setTimeout(function () {
