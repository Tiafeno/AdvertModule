...
/* Module filter */
routeDashboard.filter('fromNow', function () {
...
routeDashboard.factory('factoryRouteDashboard', function ($http, $window, $q) {
  return {
    ...
    get_nonce_field: function get_nonce_field(field) {
    ...
    verifyPassword: function verifyPassword($pass) {
      return $http.get(jsDashboard.ajax_url, {
        params: {
    ...
    $httpPostForm: function $httpPostForm(form) {
      return $http({
...
routeDashboard.controller('Dashboard_EditCtrl', function ($scope, $window, factoryRouteDashboard) {
  ...
  var initialize = function initialize() {
    ...
    if ($scope.profil.img_url == null || false === $scope.profil.img_url) $scope.profil.img_url = jsDashboard.assets_plugins_url + 'img/no-avatar-male.jpg';
  };
  ...
  $scope.EventSubmit = function ($event) {
    ...
    factoryRouteDashboard.get_nonce_field(nonceField).then(function (results) {
      if (results.data.nonce == undefined) {
        console.warn('Missing nonce!');
        return false;
      }
      formdata.append('inputNonce', results.data.nonce);
      factoryRouteDashboard.$httpPostForm(formdata).then(function successCallback(results) {
        var data = results.data;
        if (data.reload != undefined && data.reload) $window.setTimeout(function () {
          location.reload();
        }, 1500);
  ...
  $scope.EventformProfilValidate = function ($event) {
    return $scope.profilForm.$dirty && $scope.profilForm.$valid ? false : $scope.profilForm.$invalid ? $scope.profilForm.$dirty ? true : false : true;
  };
  ...
  $scope.EventClickchangeAvatar = function () {
    ...
    factoryRouteDashboard.get_nonce_field('avatar_upload').then(function (results) {
  ...
  $scope.EventChangePassword = function (isValid) {
    if (isValid) {
      ...
      factoryRouteDashboard.$httpPostForm(httpform).then(function success(results) {
        var $data = results.data;
