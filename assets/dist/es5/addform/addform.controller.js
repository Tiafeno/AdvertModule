...
app.controller('AdvertFormAddCtrl', function ($scope, $http, $log, $window, $element, alertify, factoryServices) {
  ...
  */
  ...
  var validateFileExtension = function validateFileExtension(file) {
  ...
  */
  $scope.uploadFile = function () {
    ...
    $scope.$apply();
    factoryServices.httpPostFormdata(formdata).then(function (resp) {
      var dataResponse = resp.data;
      if (resp.status != undefined && resp.status == 200) {
        if (true === dataResponse.type) {
          if (setFirstImage) $scope.imageGallery.push({ file: dataResponse.url, id: dataResponse.attach_id });
          if (!setFirstImage) {
            setFirstImage = true;
          }
  ...
  $scope.setFormSubmit = function (isValid) {
    ...
    $scope.imageGallery.forEach(function (el) {
      Gallery.push(el.id);
    });
    $scope.advertPost.hidephone = $scope.advertPost.hidephone == true ? 1 : 0;
    var attrs = [];
    _.map($scope.advertPost.attributs, function (val, key) {
      if ($scope.optionalInput[key]) attrs.push({ 'value': val, '_id': key }); // e.g {"value":"0","_id":"real_estate_type"}
    });
    ...
    factoryServices.httpPostFormdata(advertdata).then(function (results) {
      ...
      $window.setTimeout(function () {
        if (data.type) $scope.$apply(function () {
  ...
  $scope.onClickDeleteThumb = function (post_id) {
    if (typeof post_id == "number") {
      alertify.okBtn("Oui").cancelBtn("Non").confirm('Voulez vous vraiment effacer cette image?', function (ev) {
        /* ok */
        ev.preventDefault();
        ...
        factoryServices.httpPostFormdata(delete_formdata).then(function (results) {
          var data = results.data;
          if (!data.type) {
            console.warn(data);return $scope.Progress.Image = false;
          }
          $scope.Images = _.reject($scope.Images, function (Image) {
            return Image.id === data.ID;
          });
          $scope.imageGallery = _.reject($scope.imageGallery, function (gallery) {
  ...
  this.Initialise = function () {
    ...
    factoryServices.getTermsProductCategory().then(function (results) {
      ...
      console.warn('Terms products error');
    });
